...
env_keys: POSTGRES_ENV_TK | MONGO_ENV_TK | MYSQL_ENV_TK;
env_values: STRING;
...
bst_keys: (FLAG_TOKEN | IMAGE_TYPE_TK | CREATE_TK | STATUS_TK);
bst_values: flag_val | image_type | NUMBER | STRING;
type_command: ( DOWN_TK |  UP_TK );
flag_val: ( RUNNING_TK| ALL_TK | EXITED_TK );
image_type: ('postgres' | 'mongo' | 'nginx' | 'django' | 'rabbitmq' | 'mysql' | 'timescale/timescaledb:latest-pg11');
POSTGRES_ENV_TK: ('POSTGRES_USER' | 'POSTGRES_PASSWORD' | 'POSTGRES_DB');
MONGO_ENV_TK: ('MONGO_TK');
MYSQL_ENV_TK: ('MYSQL_ROOT_PASSWORD' | 'MYSQL_DATABASE' | 'MYSQL_USER' | 'MYSQL_PASSWORD' | 'MYSQL_ALLOW_EMPTY_PASSWORD');
PERFORM_TK: 'perform ';
SET_TK: 'set';
UP_TK: 'up';
DOWN_TK: 'down';
